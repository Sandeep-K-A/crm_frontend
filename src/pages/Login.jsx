...
import { loginFormSchema } from "../validators/authSchemas.js";
...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setApiError(err.response?.data?.message || "Unable to log in. Please try again.");
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
      <div className="w-full max-w-sm rounded-xl bg-white p-8 shadow-sm ring-1 ring-slate-200">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">Log in to your CRM dashboard</p>
        </div>
        ...
        <p className="mt-5 text-center text-sm text-slate-500">
          ...
          <Link to="/register" className="font-medium text-[--color-brand] hover:underline">
            Register
          </Link>
        </p>
      </div>
